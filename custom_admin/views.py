...
def reset_platform(request):
    """ONE-TIME: Wipes ALL data and creates hashim admin. Token = settings.SECRET_KEY."""
    token = request.GET.get('token', '')
    expected = os.environ.get('RESET_SECRET') or settings.SECRET_KEY
    if not token or token != expected:
        return JsonResponse({'error': 'Invalid or missing reset token. Pass ?token=<SECRET_KEY>'}, status=403)

    results = {'status': 'ok', 'steps': {}}

    # 1. Clean Supabase Storage
    try:
        from accounts.utils.supabase_storage import supabase, bucket_name, resource_supabase, resource_bucket_name
        if supabase:
            for bname in [bucket_name, resource_bucket_name]:
                try:
                    files = supabase.storage.from_(bname).list()
                    if files:
                        paths = [f['name'] for f in files]
                        supabase.storage.from_(bname).remove(paths)
                except Exception:
                    pass
        if resource_supabase and resource_bucket_name:
            try:
                files = resource_supabase.storage.from_(resource_bucket_name).list()
                if files:
                    paths = [f['name'] for f in files]
                    resource_supabase.storage.from_(resource_bucket_name).remove(paths)
            except Exception:
                pass
        results['steps']['supabase'] = 'cleaned'
    except Exception as e:
    ...
    try:
        ...
        results['steps']['cloudinary'] = 'cleaned'
    except Exception as e:
    ...
    try:
        ...
        results['steps']['db_truncated'] = f'{len(tables)} tables'
    except Exception as e:
    ...
    try:
        ...
        admin_user = CustomUser.objects.create_user(
            ...
            is_superuser=True,
            is_active=True,
            full_name='Hashim Admin',
        )
        results['steps']['admin_created'] = 'hashim'
        results['admin_username'] = 'hashim'
        results['admin_password'] = admin_pw
    except Exception as e:
        results['steps']['admin_created'] = f'error: {e}'
