...
def startup_check():
    print("🚀 EduStream Startup Guard: Initializing...")
    ...
    if os.path.exists(heartbeat_file):
        with open(heartbeat_file, 'r') as f:
            try:
                ...
                if hours_since > 25:
                    send_alert(
                        "🚩 EduStream Startup Guard: BACKUP IS STALE",
                        f"The system just started up, but the last successful backup was {hours_since:.1f} hours ago ({last_dt})."
                    )
            except: pass
    else:
        send_alert("🚩 EduStream Startup Guard: BACKUP NEVER SUCCEEDED", "No successful backup record found.")
